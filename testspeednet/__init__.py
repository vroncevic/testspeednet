...
try:
    ...
    from testspeednet.net import TestSpeedNetProcessor
except ImportError as ats_error_message:
    # Force close python ATS ##################################################
    sys.exit(f'\n{__file__}\n{ats_error_message}\n')

__author__ = 'Vladimir Roncevic'
__copyright__ = 'Copyright 2022, https://vroncevic.github.io/testspeednet'
...
class TestSpeedNet(CfgCLI):
    '''
        ...
        It defines:
            ...
            :methods:
                | __init__ - Initials TestSpeedNet constructor.
                | process - Processes and runs operations.
    '''

    _TOOL_VERBOSE: str = 'TEST_SPEED_NET'
    _CONFIG: str = '/conf/testspeednet.cfg'
    _LOG: str = '/log/testspeednet.log'
    _LOGO: str = '/conf/testspeednet.logo'
    _OPS: List[str] = ['-c', '--cmd', '-v', '--verbose']

    def __init__(self, verbose: bool = False) -> None:
        '''
            Initials TestSpeedNet constructor.
        ...
        testspeednet_property: Dict[str, str | bool] = {
        ...
        Splash(testspeednet_property, verbose)
        ...
        if self.tool_operational:
            self.add_new_option(
                ...
                required=True,
                help='check net speed (download/upload)'
            )
            self.add_new_option(
            ...
            try:
                ...
                try:
                    ...
                    match args:
                        ...
                        case Namespace(cmd='download'):
                            command = getattr(args, 'cmd')
                        case Namespace(cmd='upload'):
                            command = getattr(args, 'cmd')
                        case Namespace(cmd='fetch'):
                            command = getattr(args, 'cmd')
                        case _:
                            error_message(
                                [
                    ...
                    if bool(command):
                        status = tool.execute(
                            command, getattr(args, 'verbose') or verbose
                        )
                ...
                if status:
                    ...
                    self._logger.write_log(
                        'execute command done', self._logger.ATS_INFO
                    )
                else:
                    error_message([f'{self._TOOL_VERBOSE.lower()} failed'])
                    self._logger.write_log(
                        'execute command failed', self._logger.ATS_ERROR
                    )
            except SystemExit:
                error_message(
